...
class TestConditionedLoFiModel:
    ...
    def test_conditioning_token_ranges(self, test_config):
        ...
        key_range = range(
            model.key_token_start,
            model.key_token_start + model.key_token_count
        )
        mood_range = range(
    ...
    def test_get_tempo_token(self, test_config):
        ...
        for tempo in test_tempos:
            ...
            assert model.tempo_token_start <= token < \
                   model.tempo_token_start + model.tempo_token_count
    ...
    def test_get_key_token(self, test_config):
        ...
        for key in keys:
            ...
            assert model.key_token_start <= token < \
                   model.key_token_start + model.key_token_count
    ...
    def test_get_mood_token(self, test_config):
        ...
        for mood in moods:
            ...
            assert model.mood_token_start <= token < \
                   model.mood_token_start + model.mood_token_count
        ...
        assert model.mood_token_start <= token_unknown < \
               model.mood_token_start + model.mood_token_count
    ...
    def test_create_conditioning_prefix(self, test_config):
        ...
        prefix = model.create_conditioning_prefix(
            tempo=75,
            key='C',
            mood='chill'
        )
        ...
        assert model.tempo_token_start <= tempo_token < \
               model.tempo_token_start + model.tempo_token_count
        assert model.key_token_start <= key_token < \
               model.key_token_start + model.key_token_count
        assert model.mood_token_start <= mood_token < \
               model.mood_token_start + model.mood_token_count
