...
class TestEndToEndPipeline:
    ...
    def test_tokenize_generate_pipeline(self, test_config, sample_midi_path, temp_dir):
        ...
        metadata = generator.generate_and_save(
            str(output_path), tempo=75, key="C", mood="chill", max_length=256  # Shorter for testing
        )
    ...
    def test_full_audio_pipeline(self, test_config, sample_midi_path, temp_dir):
        ...
        metadata = generator.generate_and_save(str(midi_output), max_length=256)
        ...
        if "output_path" in metadata:
            ...
            audio_result = audio_processor.process_midi_to_lofi(
                metadata["output_path"], str(audio_output_dir), save_clean=True, save_lofi=True
            )
    ...
    def test_conditioned_generation_pipeline(self, test_config, temp_dir):
        ...
        tokens, metadata = generator.generate_track(
            tempo=75, key="Am", mood="melancholic", max_length=128
        )
    ...
    def test_batch_generation_pipeline(self, test_config, temp_dir):
        ...
        metadata_list = generator.batch_generate(
            num_tracks=3, output_dir=str(output_dir), ensure_variety=True
        )
    ...
    def test_directory_tokenization_pipeline(self, test_config, sample_midi_path, temp_dir):
        ...
        stats = tokenizer.tokenize_directory(str(input_dir), str(output_dir), check_quality=False)
    ...
    def test_quality_filtering_pipeline(self, test_config, temp_dir):
        ...
        for i in range(200):
            note = pretty_midi.Note(
                velocity=80, pitch=60 + (i % 12), start=i * 0.2, end=(i + 0.5) * 0.2
            )
            piano.notes.append(note)

        for i in range(100):
            note = pretty_midi.Note(velocity=100, pitch=36, start=i * 0.4, end=(i + 0.1) * 0.4)
        ...
        stats = tokenizer.tokenize_directory(str(input_dir), str(output_dir), check_quality=True)
    ...
    def test_complete_production_pipeline(self, test_config, sample_midi_path, temp_dir):
        ...
        stats = tokenizer.tokenize_directory(str(input_dir), str(tokens_dir), check_quality=False)
        ...
        metadata_list = generator.batch_generate(
            num_tracks=2, output_dir=str(output_dir), ensure_variety=True
        )
        ...
        for metadata in metadata_list:
            if "output_path" in metadata:
                result = audio_processor.process_midi_to_lofi(
                    metadata["output_path"], str(audio_dir), save_lofi=True, save_clean=False
                )
