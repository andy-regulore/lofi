...
class AmbientSoundGenerator:
    ...
    def generate_rain(
        self, duration: float, intensity: str = "medium", include_thunder: bool = False
    ) -> np.ndarray:
        """
        ...
        intensities = {
            "light": {"density": 0.3, "pitch_range": (2000, 8000), "amplitude": 0.1},
            "medium": {"density": 0.5, "pitch_range": (1500, 6000), "amplitude": 0.15},
            "heavy": {"density": 0.8, "pitch_range": (1000, 5000), "amplitude": 0.25},
        }
        ...
        b, a = signal.butter(
            4, [low / (self.sample_rate / 2), high / (self.sample_rate / 2)], "band"
        )
    ...
    def generate_cafe_ambience(self, duration: float, busyness: str = "medium") -> np.ndarray:
        """
        ...
        busyness_levels = {"quiet": 0.5, "medium": 1.0, "busy": 2.0}
        ...
        for _ in range(num_bursts):
            ...
            b_speech, a_speech = signal.butter(
                2, [200 / (self.sample_rate / 2), 3000 / (self.sample_rate / 2)], "band"
            )
    ...
    def generate_nature_sounds(self, duration: float, environment: str = "forest") -> np.ndarray:
        """
    ...
    def _generate_forest_sounds(self, duration: float, samples: int) -> np.ndarray:
        ...
        b, a = signal.butter(
            2, [200 / (self.sample_rate / 2), 2000 / (self.sample_rate / 2)], "band"
        )
        ambience = signal.filtfilt(b, a, ambience)
...
class AmbientSoundMixer:
    ...
    def add_ambient_to_track(
        self, audio: np.ndarray, ambient_type: str, mix_level: float = 0.15, **ambient_params
    ) -> np.ndarray:
        """
...
def add_ambient_to_file(
    ...
    **ambient_params,
    ...
    if audio.ndim == 2:
        # Mix both channels
        mixed_left = mixer.add_ambient_to_track(audio[0], ambient_type, mix_level, **ambient_params)
        ...
        # Mono
        mixed = mixer.add_ambient_to_track(audio, ambient_type, mix_level, **ambient_params)
...
AMBIENT_PRESETS = {
    ...
    "cafe": {"ambient_type": "cafe", "busyness": "medium", "mix_level": 0.10},
    "forest": {"ambient_type": "nature", "environment": "forest", "mix_level": 0.12},
    "beach": {"ambient_type": "nature", "environment": "beach", "mix_level": 0.15},
    "windy": {"ambient_type": "nature", "environment": "wind", "mix_level": 0.10},
}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--preset", choices=list(AMBIENT_PRESETS.keys()), default="rainy_day", help="Ambient preset"
    )
