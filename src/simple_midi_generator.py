...
class SimpleMidiGenerator:
    ...
    def generate(
        self, duration: int = 180, output_path: Optional[str] = None
        ...
        for bar in range(num_bars):
            ...
            bass_pattern = self._generate_bass_pattern(
                bass_note, start_time, bar_duration, beat_duration
            )
            ...
            if bar % 2 == 1 or self.mood == "upbeat":
                melody_notes = self._generate_melody(
                    chord_intervals, start_time, bar_duration, beat_duration
                )
    ...
    def _generate_bass_pattern(
        self, root: int, start_time: float, bar_duration: float, beat_duration: float
...
if __name__ == "__main__":
    ...
    generate_lofi_midi(key="Am", mood="chill", tempo=75, duration=30, output_path="test_lofi.mid")
